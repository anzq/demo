    ...
    <div class="App-3Q8Qb">
      ...
      <div>
         <form v-if="islongin">
          ...
          <button class="SubmitButton-2wG4T" @click="longinClick()">登陆</button>
        </form> 
        <form v-else>
          <section class="form-b6px1">
            <input type="text" autocapitalize="on" placeholder="手机/邮箱/用户名" ref='phone'>
          </section>
          <section class="form-b6px1">
            <input placeholder="密码" type="password" autocapitalize="on" ref="password">
          </section>
          <button class="SubmitButton-2wG4T" @click="longinClick()">登陆</button>
        ...
        if (/^1[3|4|5|8][0-9]\d{8}$/.test(phone)) {
          if (password !== '') {
            axios.post('http://localhost:3000/user', {
              ...
              console.log(res)
              // 返回数据判断  0====成功匹配允许登陆   2======匹配失败  后台自动注册
              if (res.data[0].status === 0) {
                sessionStorage.setItem('userInfo', JSON.stringify({id: '匿名', phone: phone}))
                that.$router.push('/user?top_title=我的')
              } else if (res.data[0].status === 2) {
                that.title = '已自动注册，请重新登录'
              }
            })
          } else {
            that.title = '密码不可以为空'
